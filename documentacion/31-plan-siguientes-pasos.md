...
> **Estado (2026-08-13): el punto 13 está HECHO** — Kakashi y los cascabeles, con la calibración
> medida y documentada. Se hizo antes del playtest porque el bloque A (jugar) es del usuario y este
> era el bloque de reserva previsto justo para eso. Su desarrollo está en
> [05-roadmap.md](./05-roadmap.md) sección "Hecho".
>
> **El 7 (playtest) queda aparcado, no descartado**: no hay tiempo de jugar ahora, y los resultados se
> irán anotando en el roadmap cuando los haya. Sigue siendo el punto con más valor por hora invertida
> —las dos últimas tandas de mejoras salieron enteras de partidas reales— y ahora hay un motivo nuevo:
...
| 3.º | **10 — Enciclopedia** | Deuda real de información, no un "ya lo pondremos": hoy esa información **no está en ninguna parte**. Y el punto 13 la agranda | Media-grande | No (solo lectura) |
| 4.º | **5 — Logros** | El más grande de todos, y lo grande no es la pantalla sino el contenido y los sistemas que da por supuestos (ver abajo) | Grande, partido en 3 | Sí — `achievements.json`, `engine/achievements.js`, store |
...
- **5a — contenido y condiciones.** Campo `categoria` en `achievements.json`, contadores acumulados
  en `useAchievementsStore` (combates, reclutas, oro total) y los tipos de condición que los usan.
  Es lo que llena la pantalla; sin esto lo demás no tiene qué enseñar.
...
- **5c — la pantalla**, siguiendo el doc 25 tal cual. Es la parte fácil y la última.
...
Y de paso, en 5c: **quitar el botón `[DEV] Reset achievements`** de `AchievementsScreen.jsx`, que
está anotado como pendiente en `CLAUDE.md` desde hace tiempo.
...
2. **Sprite de columna más simple y más representativo del arco.** Se regenera con
   ...
   editan a mano.
3. **El nombre del arco con el estilo Naruto**: negro con reborde blanco. Se hace con
   `-webkit-text-stroke` o con capas de `text-shadow` sobre la fuente `font-naruto` que ya existe.
...
1. `npm test` — **obligatorio** si el cambio toca `engine/` o `store/`. Son 190 tests y existen para
